...
float Random(int add) {
    //add = (bounce + add) % 4;
    int x = get_global_id(0);
    int y = get_global_id(1);
    int z = sample;
    int loc = (get_global_id(1) * renderData[0].w + get_global_id(0)) * 4 * sample + add * sample;
    //int loc = (z * renderData[0].w * renderData[0].h) + (y * renderData[0].w) + x;
    //loc *= 4;
    //loc += add;
    return randomBuffer[loc];
}
...
float RandomNoise(float x, float y)
{
    ...
    float val = sin(x * 12.9898 * i + y * 78.233 * j) * 43758.5453;
    return fract(val);
}
...
Vector3 TracePath(Vector3 origin, Vector3 dir)
{
    ...
    for (int i = 0; i < renderData[0].bounces; i++)
    {
        ...
        if (hit)
        {
            ...
            Vector3 randVec = {
                    RandomNoise(bounce+555, sample+1) * 2.0 - 1.0,
                    RandomNoise(bounce+666, sample+2) * 2.0 - 1.0,
                    RandomNoise(bounce+777, sample+3) * 2.0 - 1.0,
            };
            ...
            bool refracted = material.opacity > RandomNoise(bounce+888, sample+4);
            if (refracted)
            {
...
kernel void Render(__global float * RD, __global float * screen, __global float * rnd_buffer)
{
    randomBuffer = rnd_buffer;
